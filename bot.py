...
import locale # Импортирование модуля locale, который позволяет устанавливать и использовать локальные настройки (язык, кодировку, формат даты и времени и т.д.) в программе
...
TOKEN = "6205531190:AAGk_Twj6LlG7RfMED42x2l_Ju_rED_VlBs" # Ваш личный токен бота
...
async def lesson_handler(message: types.Message): # Обработчик команды /lesson, который находит ближайшее занятие
    ...
    locale.setlocale(locale.LC_ALL, 'ru_RU.UTF-8') # Устанавливам локальные настройки для всех категорий ( LC_ALL ) на русский язык с кодировкой UTF-8
    ...
    if next_lesson: # Если переменная next_lesson не равна None
        ...
        next_lesson_date = next_lesson['date'].strftime('%A, %d %B %Y') # В переменную next_lesson_date записывается дата занятия в формате День недели, ЧИСЛО МЕСЯЦ ГОД
        response_text = f"<b>Ближайшее занятие:</b>\n <ins>Дата:</ins> {next_lesson_date}\n <ins>Время:</ins> {next_lesson_time} МСК\n <ins>Урок:</ins> {next_lesson['lesson']}\n <ins>Преподаватель:</ins> {next_lesson['teacher']}." # Переменной присваевается данные ближайшего занятие с датой, временем, уроком и преподавателем
    ...
    await message.answer(response_text, parse_mode="HTML")  # В конце функции выдаем сообщение с текстом ответа отправляется пользователю, который вызвал команду
...
async def week_handler(message: types.Message): # Обработчик команды /week, который отправляет расписание на текущую неделю
    ...
    if not week_lessons: # Если занятий не найдено
        ...
        response_text = f'Расписание на текущую неделю с <b>{week_start.date().strftime("%d-%m-%Y")}</b> по <b>{week_end.date().strftime("%d-%m-%Y")}</b>:\n\n' # Записываем расписание на текущую неделю с начальной даты недели по конечную
        for lesson in week_lessons: # Перебираем все занятия на этой неделе
            lesson_date = lesson['date'].strftime('%a, %d %B') # В переменную lesson_date записывается дата занятия в формате День недели, Число Месяц
            lesson_time = lesson['time'].strftime('%H:%M') # В переменную lesson_time записывается время занятия в формате ЧАСЫ:МИНУТЫ
            response_text += f'<b>{lesson_date} {lesson_time} МСК</b> - {lesson["lesson"]}\n' # Формируется список занятий в формате дата, время и урок
    await message.answer(response_text, parse_mode="HTML") # В конце функции выдаем сообщение с текстом ответа отправляется пользователю, который вызвал команду

@dp.message_handler(commands=["nextweek"]) # Декоратор, указывающий на то, что обработчик сообщений будет отвечать только на команду /nextweek
async def nextweek_handler(message: types.Message): # Обработчик команды /nextweek, который отправляет расписание на следующую неделю
    user_id = message.from_user.id # Получение id пользователя, отправившего сообщение
    user_full_name = message.from_user.full_name # Получение полного имени пользователя, отправившего сообщение
    logging.info(f'{user_id} {user_full_name} {time.asctime()}') # Запись информации о пользователе в логи приложения в формате: id пользователя, полное имя пользователя и время отправки сообщения
    ...
    now = datetime.datetime.now(tz) # Получаем текущее время now и присваиваем ему значение текущего дня и времени в указанном часовом поясе
    locale.setlocale(locale.LC_ALL, 'ru_RU.UTF-8') # Устанавливам локальные настройки для всех категорий ( LC_ALL ) на русский язык с кодировкой UTF-8
    week_start = now + datetime.timedelta(days=7-now.weekday()) # Определяем дату начала следующей недели
    week_end = week_start + datetime.timedelta(days=6) # Определяем дату окончания следующей недели

    week_lessons = [lesson for lesson in timetable # Фильтруем занятия из списка "timetable" 
                    if week_start.date() <= lesson['date'] <= week_end.date()] # Выбирая только те, которые проходят на следующей неделе
    if not week_lessons: # Если занятий не найдено
        response_text = 'На следующей неделе занятий нет' # Записываем в переменную, что занятий нет на этой неделе
    else: # Иначе
        response_text = f'Расписание на следующую неделю с <b>{week_start.date().strftime("%d-%m-%Y")}</b> по <b>{week_end.date().strftime("%d-%m-%Y")}</b>:\n\n' # Записываем расписание на следующую неделю с начальной даты недели по конечную
        for lesson in week_lessons: # Перебираем все занятия на следующей неделе
            lesson_date = lesson['date'].strftime('%a, %d %B') # В переменную lesson_date записывается дата занятия в формате День недели, Число Месяц
            lesson_time = lesson['time'].strftime('%H:%M') # В переменную lesson_time записывается время занятия в формате ЧАСЫ:МИНУТЫ
            response_text += f'<b>{lesson_date} {lesson_time} МСК</b> - {lesson["lesson"]}\n' # Формируется список занятий в формате дата, время и урок
    await message.answer(response_text, parse_mode="HTML") # В конце функции выдаем сообщение с текстом ответа отправляется пользователю, который вызвал команду
